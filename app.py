"""Spain Midfielders — xT, passes and carries analysis (Rodri, Fabián Ruiz, Dani Olmo)."""
...
TARGET_PLAYER_IDS: tuple[str, ...] = ("827606", "784655", "789071")
TARGET_PLAYER_ORDER: tuple[tuple[str, str], ...] = (
    ("827606", "Rodri"),
    ("784655", "Fabián Ruiz"),
    ("789071", "Dani Olmo"),
)

APP_NAME = "Spain Midfielders"
APP_LEAGUE = "Spain vs France"
MATCH_LABEL = "France 0–2 Spain · 14 Jul 2026"
...
st.markdown(
    ...
    .pa-triple-col .pa-layout {
        grid-template-columns: 1fr;
        gap: 0.55rem;
    }
    .pa-triple-col .pa-col {
        display: flex;
        flex-direction: column;
    }
    .pa-triple-col .pa-score-stack {
        height: auto;
        min-height: 0;
        max-height: none;
    }
    .pa-triple-col .pa-identity-card,
    .pa-triple-col .pa-rating-panel,
    .pa-triple-col .radar-card,
    .pa-triple-col .pa-pillars-card {
...
def _build_player_analysis_left_card_html(
    ...
    *,
    ...
    group_label = position_group_label(str(player.get("position_group") or "midfielders"))
    badges = _rating_badges_html(player)
...
def render_player_analysis_section(
    ...
    for player_id, _name in TARGET_PLAYER_ORDER:
        player = _resolve_progression_analysis_player(
            player_id,
            progression_by_id,
            pass_by_id,
        ...
        if player is None:
            columns_html.append(
                f'<div class="pa-triple-col"><div class="player-card">'
                f'<p>Perfil indisponível para {_name}.</p></div></div>'
            )
            continue
    ...
    st.html(
        f'<div class="pa-triple-layout">{"".join(columns_html)}</div>',
        width="stretch",
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
def render_presentation_tab(
    ...
    *,
    ...
    st.markdown(
        ...
        f"<h4>{html.escape(APP_NAME)} — análise de xT, passes e conduções</h4>"
        f"<p>Análise dos meio-campistas da <strong>Espanha</strong> na partida "
        f"<strong>{html.escape(MATCH_LABEL)}</strong>. "
        "Medimos a qualidade dos passes e conduções com um modelo de "
